...
if __name__ == '__main__':
    ...
    db = DatasetBuilder()
    pm = PolicyModel(num_layer=10, num_ops=len(op_list), num_features=len(feature_list), lstm_size=256, 
                    lstm_num_layers=2, tanh_constant=1.0, temperature=1.0)

    optimizer = torch.optim.Adam(pm.parameters(), lr=0.01)
